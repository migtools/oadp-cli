...
func NewDeleteCommand(f client.Factory, use string) *cobra.Command {
	...
	c := &cobra.Command{
		...
		Long:  "Delete one or more non-admin restores. Use --all to delete all restores in the current namespace.",
...
type DeleteOptions struct {
	...
	All       bool   // Delete all restores in namespace
...
func (o *DeleteOptions) BindFlags(flags *pflag.FlagSet) {
	...
	flags.BoolVar(&o.All, "all", false, "Delete all restores in the current namespace")
}
...
func (o *DeleteOptions) Complete(args []string, f client.Factory) error {
	...
	// If --all flag is used, list all restores in the namespace
	if o.All {
		var narList nacv1alpha1.NonAdminRestoreList
		err := o.client.List(context.TODO(), &narList, &kbclient.ListOptions{
		...
		// Extract restore names
		o.Names = make([]string, 0, len(narList.Items))
		for _, nar := range narList.Items {
			o.Names = append(o.Names, nar.Name)
		}

		if len(o.Names) == 0 {
			return fmt.Errorf("no restores found in namespace '%s'", o.Namespace)
		}
	}
...
func (o *DeleteOptions) Validate() error {
	if !o.All && len(o.Names) == 0 {
		return fmt.Errorf("at least one restore name is required, or use --all to delete all restores")
	}
	if o.Namespace == "" {
...
func (o *DeleteOptions) Run() error {
	...
	if o.All {
		fmt.Printf("All NonAdminRestore(s) in namespace '%s' will be deleted:\n", o.Namespace)
	} else {
		fmt.Printf("The following NonAdminRestore(s) will be deleted in namespace '%s':\n", o.Namespace)
	}
...
func (o *DeleteOptions) promptForConfirmation() (bool, error) {
	...
	if o.All {
		fmt.Printf("Are you sure you want to delete ALL %d restore(s) in namespace '%s'? (y/N): ", len(o.Names), o.Namespace)
